...
export async function registerActions(){
    ...
    opendiscord.actions.get("opendiscord:reopen-ticket").workers.add([
        new api.ODWorker("opendiscord:reopen-ticket",2,async (instance,params,origin,cancel) => {
            ...
            if (params.sendMessage) await channel.send((await opendiscord.builders.messages.getSafe("opendiscord:reopen-message").build(origin,{guild,channel,user,ticket,reason})).message)
